...
namespace JamUp.Waves.RuntimeScripts
{
    ...
    public struct CurrentWavesElement: IBufferElementData, IValueSettable<float4x4>, IRequiredInArchetype
    {
        ...
        public float PhaseDelta(float lerpTime, float deltaTime) => (LerpPhase(lerpTime - deltaTime) - LerpPhase(lerpTime + deltaTime)) / (2 * deltaTime);
        ...
        public float ValueAtPhase(float phase, float lerpTime)
        {
            ...
            return math.dot(waveTypeRatio, amplitude * new float4(sineValue, squareFactor, triangleFactor, sawToothFactor));
        }
